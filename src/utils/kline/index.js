...
var Kline = function (option) {
  ...
  this.language = "zh-cn";
  this.theme = "dark";
  this.ranges = ["1M","7d", "1d", "12h", "6h", "4h", "2h", "1h", "30m", "15m",  "5m", "line"];
  this.showTrade = true;
  this.tradeWidth = 250;
  ...
  this.periodMap = { "1M": "1M", "7d": "7d", "1d": "1d", "12h": "12h", "6h": "6h", "4h": "4h", "2h": "2h", "1h": "1h", "30m": "30m", "15m": "15m", "5m": "5m", line: "line" };

  this.tagMapPeriod = { "1M": "1M", "7d": "7d", "1d": "1d", "12h": "12h", "6h": "6h", "4h": "4h", "2h": "2h", "1h": "1h", "30m": "30m", "15m": "15m", "5m": "5m", line: "line" };
...
KlineTrade.prototype = {
  ...
  updateDepth: function (data) {
    ...
    if (!data) return;
    if(!(data.hasOwnProperty('asks'))) return;
...
DarkTheme.prototype.__construct = function () {
  ...
  if (KlineIns.reverseColor) {
    this._colors[Theme.Color.Positive] = "#e64c65"; 
    this._colors[Theme.Color.Negative] = "#29bc86";
    this._colors[Theme.Color.PositiveDark] = "#3b0e08";
    this._colors[Theme.Color.NegativeDark] = "#004718";
  } else {
    this._colors[Theme.Color.Positive] = "#29bc86";
    this._colors[Theme.Color.Negative] = "#e64c65";
  ...
  this._colors[Theme.Color.Cursor] = "#5a7091";
  this._colors[Theme.Color.RangeMark] = "#6a2861";
  this._colors[Theme.Color.Indicator0] = "#5a7091";
  this._colors[Theme.Color.Indicator1] = "#6a2861";
  ...
  this._colors[Theme.Color.Grid0] = "#a6a6a6";
  this._colors[Theme.Color.Grid1] = "#a6a6a6";
  ...
  this._colors[Theme.Color.LineColorSelected] = "#5a7091";
  this._colors[Theme.Color.CircleColorFill] = "#000000";
  this._colors[Theme.Color.CircleColorStroke] = "#5a7091";
  this._fonts = [];
...
CandlestickPlotter.prototype.Draw = function (context) {
  ...
  for (var i = start; i < last; i++) {
    ...
    if (close > open) {
      ...
      if (iH > 1 && iW > 1 && dark)
        strokePosRects.push({x: left + 1, y: top + 1, w: iW - 1, h: iH - 1});
      else
    ...
    for (var i = first; i < last; i++) {
      ...
      if (MACD >= 0) {
        ...
        if ((i == 0 || MACD >= prevMACD) && iH > 1 && iW > 1)
          strokePosRects.push({x: left + 1, y: middle - iH + 1, w: iW - 1, h: iH - 1});
        else
          fillPosRects.push({x: left, y: middle - iH, w: Math.max(iW, 1), h: Math.max(iH, 1)});
      } else {
        var iH = range.toHeight(-MACD);
        if ((i == 0 || MACD >= prevMACD) && iH > 1 && iW > 1)
          strokeNegRects.push({x: left + 1, y: middle + 1, w: iW - 1, h: iH - 1});
        else
...
ChartSettings.checkVersion = function () {
  if (ChartSettings._data.ver < 2) {
    ...
    charts.period_weight["5m"] = 7;
    charts.period_weight["15m"] = 6;
    charts.period_weight["30m"] = 5;
    charts.period_weight["1h"] = 4;
    charts.period_weight["2h"] = 3;
    charts.period_weight["4h"] = 2;
    charts.period_weight["6h"] = 1;
    charts.period_weight["12h"] = 0;
    charts.period_weight["1d"] = 0;
    charts.period_weight['7d'] = 0;
    charts.period_weight['1m'] = 0;
  }
...
function requestOverSocket() {
  ...
  if (KlineIns.socketClient && KlineIns.socketClient.connected ) {
    KlineIns.socketClient.emit('kline',parseRequestParam(KlineIns.requestParam))
    ...
    KlineIns.G_HTTP_REQUEST = $.ajax({
      ...
      success: function (res) {
        ...
        var datas = new Array();
          ...
          var dataResult = {
             "success":true,"data":{"depths":{"asks":[],"bids":[]},"lines":datas,"symbol":""} };

          requestSuccessHandler(dataResult);
        }
      },
...
function requestSuccessHandler(res) {
  ...
  if (!res || !res.success) {
    KlineIns.timer = setTimeout(function () {
      console.log(" if (!res || !res.success)");
      RequestData(true);
    }, KlineIns.intervalTime);
    return;
  }
  ...
  // if (!updateDataRes) {
  //   if (KlineIns.type != "socket" || !KlineIns.socketClient) {
  //       console.log("  if (KlineIns.type != || !KlineIns.socketClient) {");
...
function TwoSecondThread() {
  ...
  if (KlineIns.type != "socket" || !KlineIns.socketClient) {
      // console.log("sdfgsfdfgsd  if (KlineIns.type != || !KlineIns.socketClient) {" );
        KlineIns.timer = setTimeout(RequestData, 5000);
    // RequestData();
  }
}
...
function setHttpRequestParam(mark_from, range, limit, since) {
                        // var str = "symbol=" + mark_from;

                        // switch(range){
                        //   case 60000:
                        //     str = str + "&dimension=ONE_MINUTE";
                        //     break;
                        //   case 300000:
                        //     str = str + "&dimension=FIVE_MINUTES";
                        //     break;
                        //   case 900000:
                        //     str = str + "&dimension=FIFTEEN_MINUTES";
                        //     break;
                        //   case 1800000:
                        //     str = str + "&dimension=THIRTY_MINUTES";
                        //     break;
                        //   case 3600000:
                        //     str = str + "&dimension=ONE_HOUR";

                        //     break;
                        //   case 86400000:
                        //     str = str + "&dimension=ONE_DAY";
                        //     break;
                        //   case 604800000:
                        //     str = str + "&dimension=ONE_WEEK";
                        //     break;
                        // }
                        // if (limit != null)
                        //   str += "&limit=" + limit;
                        ...
                        // }
                        if ("RAM/EOS"==mark_from) {
                          return "pageSize=180&dateType=" + range;
                        }else{
                          return "pageSize=180&dateType=" + range+"&code="+mark_from;
                        }
                        // console.log("mark_from = " + mark_from);
...
function mouseWheel(e, delta) {
  document.getElementById("chart_canvasGroup").onclick=onMouseEnable;
  if(isMouseEnable){
...
function socketConnect() {
  ...
  KlineIns.socketClient = io(KlineIns.url)

  if (!KlineIns.debug) {
    KlineIns.socketClient.debug = null;
  }

  KlineIns.socketClient.on('connect', function(){
    KlineIns.socketClient.emit('kline',parseRequestParam(KlineIns.requestParam));
  });

  KlineIns.socketClient.on('lines', function(data){
    requestSuccessHandler(data);
  })

  KlineIns.socketClient.on('disconnect', function(){
    KlineIns.socketClient.close();
...
var isMouseEnable = false;
function onMouseEnable(){
...
var template_str = "\n" +
  ...
  "                <div class=\"chart_dropdown kline_logo_black\" id=\"kline_logo\" style=\" width: 50px;height: 10px;margin: 7px 0px 0px 3px;text-align: center;color: white;\">firefoxotc</div>" +
  "                <!-- Periods -->\n" +
